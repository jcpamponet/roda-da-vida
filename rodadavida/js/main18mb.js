...
function Globals() {
	...
	this.showQuestions = function(data, reset, e, indexNext) {
		...
		if (data != undefined) {
			...
			$fn.setTransition('.id-1', 1, 1000);
			$fn.setTransition('.question', 1, 1000);
	...
	this.draw = function draw(values) {
        ...
        $('#left-box').html('<center style="margin-top:225px;"><img src="/img/loading.gif"/></center>');
	};
	...
	this.onClickFunction = function onClickFunction() {
		$('#btn-next').bind(
				...
				function next(e) {
					...
					$('#btn-previews').fadeIn();
					var indexNext = parseInt(e.target.dataset.next);
					...
					if (indexNext < 13) {
						$fn.showQuestions(data, true, e,
								indexNext == 12 ? indexNext + 1 : indexNext);
		...
		$('#btn-previews').bind(
				...
				function next(e) {
					$fn.getAudio();
					var indexPreviews = parseInt(e.target.dataset.previews);
		...
		$('#formEmail').bind(
				...
				function(e) {
					var email = $('#email').val();
					var mensagem = "<p><h5><strong>Área de Alavanca: </strong></h5></p>" 
                            + $fn.capitalize($('#areaAlavanca').val()) +"<p><strong>Ação: </strong> " 
...
var questions = {
    ...
    "4" : [{"position": 4, "title" : "HOBBIES E DIVERSAO"},
    ...
    "11" : [{"position": 11, "title" : "DES.INTELECTUAL"},
